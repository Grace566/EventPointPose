# -*- coding: utf-8 -*-
# @Time    : 2022/6/24 12:19
# @Author  : Jiaan Chen

# Generate DVS PointCloud MeanLabel Dataset labels from .h5 labels and save as .npy, (xy)
...
def extract_frames_labels(label_file_dir, out_dir, P_mat_cam_all, image_info):
    ...
    for frame_num in range(data_len):
        for cam in range(4):
            ...
            u = u.astype(np.int32)
            v = v.astype(np.int32)
            label_name = "S{}_session{}_mov{}_frame{}_cam{}_label{}.npy".format(sub, session, mov, frame_num, cam, "")
